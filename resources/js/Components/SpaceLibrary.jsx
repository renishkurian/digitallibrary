...
/**
 * Bounds errors within a 3D sub-tree to prevent the entire Canvas from crashing.
 */
...
export default function SpaceLibrary({ comics, onExit }) {
    ...
    if (webglError) {
        return (
            <div className="fixed inset-0 z-[100] bg-[#050508] flex flex-col items-center justify-center p-10 text-center">
                <div className="w-20 h-20 mb-8 text-[#e8003d] opacity-50">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                    </svg>
                </div>
                <h2 className="font-['Bebas_Neue'] text-4xl tracking-[8px] text-white mb-4 uppercase">WebGL Unreachable</h2>
                <p className="max-w-md text-white/40 text-[13px] leading-relaxed mb-10 font-light">
                    Your browser or system has disabled hardware acceleration. 
                    The Grand Library requires a WebGL context to construct the archive vaults.
                </p>
                <div className="flex gap-4">
                    ...
                    <button 
                        onClick={onExit} 
                        className="px-8 py-3 bg-[#e8003d] text-white rounded-xl font-bold uppercase tracking-widest hover:scale-105 transition-all pointer-events-auto"
                    >
                        Return to Grid
                    </button>
                </div>
    ...
    return (
        <div className="fixed inset-0 z-50 bg-[#050508]">
            ...
            <AnimatePresence>
                {selectedComic && (
                    <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className="absolute right-10 top-1/2 -translate-y-1/2 w-80 bg-[#0a0a1a]/90 backdrop-blur-3xl border border-teal-500/30 rounded-3xl p-8 shadow-[0_0_80px_rgba(0,0,0,0.8)] pointer-events-auto">
                        ...
                        <h2 className="text-2xl font-bold text-white mb-2 leading-tight">{selectedComic.title}</h2>
