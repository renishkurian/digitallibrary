...
export default function Duplicates({ auth, paginatedData, filters }) {
    ...
    const { data: searchData, get: getSearch } = useForm({
        q: filters.q || '',
    });
                    ...
                    <div className="flex gap-4 items-center">
                        <form onSubmit={handleSearch} className="relative w-[300px]">
                            <input
                                ...
                                value={searchData.q}
                                onChange={(e) => setSearchQuery(e.target.value) || searchData.setData('q', e.target.value)}
                            />
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl">
                            <p className="text-[#8888a0] text-sm">
                                <span className="font-bold text-blue-400">Note:</span> Duplicates are identified by their MD5 content hash. These files have identical contents even if they have different names or paths.
                            </p>
                                ...
                                <div className="bg-white/5 px-6 py-4 flex items-center justify-between border-b border-white/7">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-[10px] text-[#64748b] uppercase tracking-widest font-black">MD5 Hash</span>
                                            <span className="text-xs font-mono text-[#a0a0b8]">{group.hash}</span>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-[10px] text-[#64748b] uppercase tracking-widest font-black block">Single File Size</span>
                                            ...
                                            {group.items.map((comic) => (
                                                <tr key={comic.id} className={`group hover:bg-white/[0.04] transition-colors ${selectedIds.includes(comic.id) ? 'bg-white/[0.03]' : ''}`}>
                                                    ...
                                                    <td className="px-6 py-4 w-16">
                                                        <div className={`w-12 h-16 bg-[#0a0a0f] rounded-lg overflow-hidden border ${selectedIds.includes(comic.id) ? 'border-[#e8003d]' : 'border-white/10 group-hover:border-[#e8003d]/30'} transition-colors shadow-sm`}>
                                                            {comic.thumbnail ? (
                                                                <img 
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className="text-[10px] text-[#64748b] font-mono truncate max-w-md mt-0.5">{comic.path}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className="text-xs text-[#a0a0b8] font-mono whitespace-nowrap">{comic.size}</span>
                                                    </td>
                                                    <td className="px-6 py-4 text-right">
                                                        <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            ...
                                                            <Link 
                                                                ...
                                                                title="Go to management to edit metadata, shelf, or visibility"
                                                            >
                                                                Manage
                                                            </Link>
                                                            <a 
