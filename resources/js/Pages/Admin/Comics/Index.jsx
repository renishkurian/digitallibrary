...
const EyeIcon = () => (
    <Icon d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0" />
);
const EyeOffIcon = () => (
...
const XIcon = ({ size = 14 }) => (
    <Icon d="M18 6 6 18M6 6l12 12" size={size} stroke={2.5} />
);
const TrashIcon = () => (
    <Icon d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M10 11v6M14 11v6" />
);
const RefreshIcon = () => (
...
const SearchIcon = () => (
    <Icon
        d="M21 21l-4.35-4.35 M11 19A8 8 0 1 0 11 3a8 8 0 0 0 0 16z"
        stroke={2.5}
    />
);
...
function StatusPill({ comic }) {
    ...
    if (!comic.is_approved && !comic.is_personal)
        return (
...
function ActionBtn({ onClick, title, children, variant = "ghost" }) {
    const base =
        "w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-150 cursor-pointer flex-shrink-0";
    const variants = {
...
function AiSummaryCell({ comic, isGenerating }) {
    ...
    if (!comic.ai_summary && tags.length === 0) {
        return (
            <span className="text-[10px] font-bold uppercase tracking-wider text-[#333348]">
                —
            </span>
        );
    }
    ...
    return (
        <div className="space-y-1.5 max-w-[280px]">
            {comic.ai_summary && (
                <p
                    className="text-[11px] leading-snug text-[#b8b8d0] line-clamp-3"
                    title={comic.ai_summary}
                >
                    {comic.ai_summary}
                </p>
            )}
            <div className="flex flex-wrap items-center gap-1">
                ...
                {tags.slice(0, 3).map((tag) => (
                    <span
                        key={tag}
                        className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-300 border border-purple-500/20"
                    >
                        {tag}
                    </span>
                ))}
...
export default function Index({
    ...
    useEffect(() => {
        setGeneratingIds((prev) =>
            prev.filter((id) => {
                const comic = comics.data.find((c) => c.id === id);
                return comic && !comic.ai_summary;
            }),
        );
    }, [comics.data]);

    useEffect(() => {
        if (generatingIds.length === 0) return undefined;

        const interval = setInterval(() => {
            router.reload({ only: ["comics"], preserveScroll: true });
        }, 5000);

        const timeout = setTimeout(() => setGeneratingIds([]), 180000);
    ...
    const runSync = () =>
        syncPost(route("admin.comics.sync"), {
            onSuccess: () =>
                setSyncStatus((p) => ({ ...p, status: "running" })),
        });
    ...
    const handleEdit = (comic) => {
        ...
        setEditData({
            title: comic.title,
            shelf_ids: comic.shelves?.map((s) => s.id) ?? [],
    ...
    const approveComic = (id) =>
        router.post(
            route("admin.comics.approve", id),
            {},
            { preserveScroll: true },
        );
    const queueAiGeneration = (ids) => {
        if (!ai_enabled) {
            toast.error("Enable AI in Admin → Settings first.");
            return;
        }
    ...
    const generateAiMeta = (id) => {
        queueAiGeneration([id]);
        router.post(route("admin.comics.generate-ai", id), {}, {
            preserveScroll: true,
            onSuccess: () =>
                toast.success(
                    "AI summary queued — LLM runs in a background process (watch storage/logs/laravel.log).",
                ),
            onError: () =>
                setGeneratingIds((prev) => prev.filter((i) => i !== id)),
        });
    };
    const revokeShare = (comic, uid) =>
        router.delete(route("admin.comics.revoke-share", [comic.id, uid]), {
            preserveScroll: true,
        });
    const revokeRoleShare = (comic, rid) =>
        router.delete(
            route("admin.comics.revoke-role-share", [comic.id, rid]),
            { preserveScroll: true },
        );

    const submitRoleShare = (rid) =>
        router.post(
            route("admin.comics.share-role", sharingComic.id),
            { role_id: rid },
            {
                preserveScroll: true,
                onSuccess: () =>
    ...
    const handleSelectAll = (e) =>
        setSelectedIds(e.target.checked ? comics.data.map((c) => c.id) : []);
    ...
    const bulkApprove = () =>
        requestConfirm({
            ...
            message: `Approve ${selectedIds.length} comics?`,
            confirmText: "Approve",
            confirmStyle: "primary",
            onConfirm: () =>
                router.post(
                    route("admin.comics.bulk-approve"),
                    { ids: selectedIds },
                    {
                        preserveScroll: true,
    ...
    const bulkGenerateAiMeta = () =>
        requestConfirm({
            ...
            message: `Queue ${selectedIds.length} comics for AI summary & tagging?`,
            onConfirm: () => {
                const ids = [...selectedIds];
                queueAiGeneration(ids);
                router.post(
                    route("admin.comics.bulk-generate-ai"),
                    { ids },
                    {
                        preserveScroll: true,
                        onSuccess: () => {
                            ...
                            closeConfirm();
                            toast.success(
                                `${ids.length} comics queued for AI processing.`,
                            );
                        },
                        onError: () =>
                            setGeneratingIds((prev) =>
    ...
    const bulkTrash = () =>
        requestConfirm({
            title: "Move to Trash",
            message: `Move ${selectedIds.length} selected comics to Trash?`,
            confirmText: "Move to Trash",
            confirmStyle: "danger",
            onConfirm: () =>
                router.post(
                    route("admin.comics.bulk-trash"),
                    { ids: selectedIds },
                    {
                        preserveScroll: true,
                        onSuccess: () => {
    ...
    const bulkRestore = () =>
        requestConfirm({
            title: "Restore Comics",
            message: `Restore ${selectedIds.length} selected comics?`,
            confirmText: "Restore",
            confirmStyle: "primary",
            onConfirm: () =>
                router.post(
                    route("admin.comics.bulk-restore"),
                    { ids: selectedIds },
                    {
                        preserveScroll: true,
                        onSuccess: () => {
    ...
    const bulkForceDelete = () =>
        requestConfirm({
            ...
            message: `PERMANENTLY delete ${selectedIds.length} selected comics? This cannot be undone!`,
            confirmText: "Delete Permanently",
            confirmStyle: "danger",
            onConfirm: () =>
                router.delete(route("admin.comics.bulk-force-delete"), {
    ...
    const bulkToggleVisibility = () =>
        requestConfirm({
            ...
            message: `Toggle visibility for ${selectedIds.length} selected comics?`,
            confirmText: "Toggle",
            confirmStyle: "primary",
            onConfirm: () =>
                router.post(
                    route("admin.comics.bulk-visibility"),
                    { ids: selectedIds },
                    {
                        preserveScroll: true,
                        onSuccess: () => {
    ...
    const handleBulkShelf = (action) => {
        ...
        requestConfirm({
            ...
            onConfirm: () =>
                router.post(
                    route("admin.comics.bulk-shelves"),
                    { ids: selectedIds, shelf_id: bulkShelfId, action },
                    {
                        onSuccess: () => {
                            setSelectedIds([]);
                            setBulkShelfId("");
    ...
    const setVisibility = (v) =>
        router.get(
            ...
            { ...filters, visibility: v, approval: "all" },
            { replace: true, preserveScroll: true },
        );
    const setApprovalFilter = (a) =>
        router.get(
            route("admin.comics.index"),
            { ...filters, approval: a, visibility: "all" },
            { replace: true, preserveScroll: true },
        );
    const setShelfFilter = (s) =>
        router.get(
            route("admin.comics.index"),
            { ...filters, shelf: s },
            { replace: true, preserveScroll: true },
    ...
    const approveAllPending = () =>
        requestConfirm({
            ...
            message: "Approve ALL pending comics? This cannot be undone.",
            confirmText: "Approve All",
            confirmStyle: "primary",
            onConfirm: () =>
                router.post(
                    route("admin.comics.approve-all-pending"),
                    {},
                    { preserveScroll: true },
                ),
        });
    ...
    const autoTagAllPending = () =>
        requestConfirm({
            title: "Auto-Tag All",
            message:
                "Queue AI summaries for ALL comics missing a summary? This may take a while.",
            confirmText: "Queue All",
            confirmStyle: "primary",
            onConfirm: () =>
                router.post(
                    route("admin.comics.auto-tag-all-pending"),
                    {},
                    {
                        preserveScroll: true,
                        onSuccess: () =>
                            toast.success(
                                "Comics without summaries queued for AI processing.",
                            ),
                    },
                ),
        });
    ...
    const handleTrash = (id) =>
        requestConfirm({
            title: "Move to Trash",
            message: "Are you sure you want to move this comic to Trash?",
            confirmText: "Move to Trash",
            confirmStyle: "danger",
            onConfirm: () =>
                router.delete(route("admin.comics.destroy", id), {
                    preserveScroll: true,
    ...
    const handleRestore = (id) =>
        requestConfirm({
            title: "Restore Comic",
            message: "Restore this comic to the main library?",
            confirmText: "Restore",
            confirmStyle: "primary",
            onConfirm: () =>
                router.post(
                    route("admin.comics.restore", id),
                    {},
    ...
    const handleForceDelete = (id) =>
        requestConfirm({
            title: "Delete Permanently",
            message:
                "PERMANENTLY delete this comic and its files? This action cannot be undone!",
            confirmText: "Delete Permanently",
            confirmStyle: "danger",
            onConfirm: () =>
                router.delete(route("admin.comics.force-delete", id), {
                    preserveScroll: true,
                    onSuccess: closeConfirm,
                }),
        });
    ...
    return (
        <ComicLayout auth={auth}>
            ...
            <div className="admin-shell -mx-4 flex max-h-[calc(100dvh-8.5rem)] min-h-0 flex-col overflow-hidden rounded-2xl border border-white/[0.09] bg-gradient-to-b from-[#0c0c16] via-[#090912] to-[#050508] shadow-[0_24px_80px_rgba(0,0,0,0.55)] sm:-mx-8 lg:-mx-14">
                {/* ── HEADER BAR ──────────────────────────────────────── */}
                <div className="shrink-0 border-b border-white/[0.06] bg-[#0e0e16]/95 backdrop-blur-md">
                    ...
                    <div className="flex items-center justify-between gap-4 px-4 py-3 sm:px-6 flex-wrap">
                        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
                            ...
                            >
                                Archive{" "}
                                <span className="text-[#e8003d]">Desk</span>
                            </h1>
                            {/* Quick nav links */}
                            <nav
                                className="ml-1 flex flex-wrap items-center gap-0.5 border-l border-white/[0.06] pl-3 sm:ml-4"
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            >
                                <RefreshCw
                                    className={`h-3.5 w-3.5 shrink-0 ${syncStatus.status === "running" ? "motion-safe:animate-spin" : ""}`}
                                    strokeWidth={2.5}
                                    aria-hidden
                                />
                                Ingress
                            ...
                            <button
                                ...
                                style={{
                                    background: uploadPanelOpen
                                        ? "#e8003d"
                                        : "rgba(232,0,61,0.12)",
                                    borderColor: "rgba(232,0,61,0.4)",
                                    color: uploadPanelOpen ? "#fff" : "#e8003d",
                                }}
                            >
                                + Upload PDF
                            </button>
                        ...
                        <div className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-top-2 motion-safe:duration-200 border-t border-white/[0.06] bg-[#0a0a12] px-4 py-5 sm:px-6">
                            ...
                            >
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#666688]">
                                        PDF File *
                                    </label>
                                    <input
                                        type="file"
                                    ...
                                    />
                                    {uploadErrors.comic && (
                                        <span className="text-[#e8003d] text-xs">
                                            {uploadErrors.comic}
                                        </span>
                                    )}
                                </div>
                                        ...
                                        {shelves.map((s) => (
                                            <button
                                                ...
                                                style={{
                                                    ...
                                                    color: uploadData.shelf_ids.includes(
                                                        s.id,
                                                    )
                                                        ? "#e8003d"
                                                        : "#888",
                                ...
                                <div className="flex items-center gap-4">
                                    {[
                                        ["is_personal", "Personal", "#3b82f6"],
                                        ["generate_ai", "AI Tags", "#a855f7"],
                                    ].map(([key, label, color]) => (
                                        <label
                                            key={key}
                                            ...
                                            <Checkbox
                                                ...
                                                aria-label={
                                                    key === "is_personal"
                                                        ...
                                                        : "Generate AI tags when uploading"
                                                }
                                            />
                    ...
                    {syncStatus.status === "running" && (
                        <div className="px-6 py-4 bg-blue-500/5 border-b border-white/5 animate-in slide-in-from-top-2 duration-300">
                            ...
                            <p className="text-[10px] text-[#64748b] mt-2 italic px-1">
                                {syncStatus.progress ||
                                    "Scanning filesystem for new and duplicate comics..."}
                            </p>
                        </div>
                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-3 border-t border-white/[0.04] px-4 py-2.5 sm:px-6">
                        ...
                        {/* Tab filters */}
                        <div
                            className="flex items-center gap-0.5 rounded-lg border border-white/8 bg-white/[0.04] p-0.5"
                            role="tablist"
                            aria-label="Filter by visibility or status"
                        >
                            {TABS.map((tab) => (
                                <button
                                    key={tab.key}
                    ...
                    {selectedIds.length > 0 && (
                        <div className="flex flex-wrap items-center gap-3 border-t border-[#e8003d]/15 bg-[#e8003d]/[0.06] px-4 py-2 sm:px-6">
                            ...
                            <button
                                ...
                                className="ml-auto inline-flex h-9 w-9 items-center justify-center rounded-lg text-[#44445a] transition-colors hover:bg-white/[0.06] hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#e8003d] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a12]"
                    ...
                    <div className="overflow-x-auto [-webkit-overflow-scrolling:touch]">
                        <table className="w-full min-w-[1100px] border-collapse">
                            <caption className="sr-only">
                                Comics in the library: sortable list with
                                actions
                            </caption>
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            >
                                                {comic.thumbnail ? (
                                                    ...
                                                    <div className="flex h-full w-full items-center justify-center text-[9px] font-bold text-[#44445a]">
                                                        NA
                                                    </div>
                                                )}
                                            </Link>
                                        ...
                                        >
                                            ...
                                            >
                                                {comic.title}
                                                {comic.is_personal && (
                                                    <span className="ml-1.5 text-[9px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded border border-blue-500/25 uppercase">
                                                        Personal
                                                    </span>
                                                )}
                                            </Link>
                                            <span className="text-[10px] text-[#44445a] font-mono block truncate mt-0.5">
                                                {comic.path}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 align-top">
                                            <AiSummaryCell
                                                comic={comic}
                                                isGenerating={generatingIds.includes(
                                                    comic.id,
                                                )}
                                            />
                                        </td>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <ActionBtn
                                                    onClick={() =>
                                                        generateAiMeta(comic.id)
                                                    }
                                                    title={
                                                        ai_enabled
                                                            ? "Generate AI summary (uses configured LLM)"
                                                ...
                                                <Link
                                                    href={route(
                                                        "admin.comics.regenerate-thumbnail",
                                                        comic.id,
                                                    )}
                                                    method="post"
                                                    as="button"
                                                    ...
                                                    <>
                                                        <ActionBtn
                                                            onClick={() =>
                                                                syncPost(
                                                                    route(
                                                                        "admin.comics.toggle-visibility",
                                                                        comic.id,
                                                                    ),
                                                                    {
                                                                        preserveScroll: true,
            ...
            {editingComic && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    ...
                    />
                    <div
                        className="relative rounded-2xl w-full max-w-2xl max-h-[88vh] overflow-y-auto shadow-2xl"
                        style={{
                            background: "#0f0f1a",
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="md:col-span-2 flex flex-col gap-1.5">
                                <label className="text-[10px] font-black uppercase tracking-widest text-[#666688]">
                                    Title
                                </label>
                                <input
                                    type="text"
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <div
                                    className="rounded-lg p-3 flex flex-col gap-1.5 overflow-y-auto"
                                    style={{
                                        height: 120,
                                    ...
                                    {shelves.map((s) => (
                                        ...
                                        >
                                            <Checkbox
                                                ...
                                                onChange={() =>
                                                    toggle("shelf_ids", s.id)
                                                }
                                                aria-label={`Assign shelf ${s.name}`}
                                            />
                                            <span className="text-[12px] text-[#8888a0]">
                                                {s.name}
                                            </span>
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                >
                                    {categories.map((cat) => (
                                        ...
                                        >
                                            <Checkbox
                                                checked={editData.category_ids.includes(
                                                    cat.id,
                                                )}
                                                onChange={() =>
                                                    toggle(
                            ...
                            <div className="md:col-span-2 p-5 rounded-xl bg-white/3 border border-white/10 flex flex-col gap-4">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-[11px] font-black uppercase tracking-widest text-[#666688]">
                                        Extended Metadata
                                    </h3>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-[#44445a]">
                                            Author
                                        </label>
                                        <input
                                            type="text"
                            ...
                            <div className="md:col-span-2 mt-4 p-5 rounded-xl bg-[#e8003d]/5 border border-[#e8003d]/15 flex flex-col gap-4">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-[11px] font-black uppercase tracking-widest text-[#e8003d]">
                                        Dangerous Action: Rename PDF File
                                    </h3>
                                    <span className="text-[9px] text-[#666688] font-mono italic">
                                        Renames file on disk and in DB
                                    </span>
                                </div>
                                ...
                                <div className="flex gap-3">
                                    <div className="flex-1 flex flex-col gap-1.5">
                                        ...
                                        />
                                        {renameErrors.new_filename && (
                                            <span className="text-[#e8003d] text-[10px] font-bold">
                                                {renameErrors.new_filename}
                                            </span>
                                        )}
                                    </div>
                                    <button
                                        ...
                                        disabled={
                                            renaming ||
                                            !renameData.new_filename ||
                                            renameData.new_filename ===
                                                editingComic.filename
                                        }
                                ...
                                <label className="flex items-center gap-2.5 cursor-pointer self-start">
                                    <Checkbox
                                        ...
                                        onChange={() =>
                                            setRenameData(
                                                "update_title",
                                                !renameData.update_title,
                                            )
                                        }
                                        color="#e8003d"
                                        aria-label="Update comic title to match new filename"
                                    />
                                    <span className="text-[11px] text-[#8888a0] font-medium">
                                        Update comic title to match new filename
                                    </span>
                                </label>
                            </div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                >
                                    ...
                                    {roles
                                        .filter(
                                            (r) =>
                                                !sharingComic.shared_roles?.find(
                                                    (s) => s.id === r.id,
                                                ),
                                        )
                                        .map((r) => (
